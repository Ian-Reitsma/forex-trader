...
def quote(spread: str = "0.0001") -> Quote:
    ...
    return Quote(
        ...
        Decimal("1.1000") - half,
        Decimal("1.1000") + half,
        technical().signal_time + timedelta(seconds=1),
    )
...
def test_fusion_requires_structure_entry_state_and_fresh_quote() -> None:
    setup = technical()
    missing_shift = replace(setup, structure_shift=False)
    result = SignalFusionPolicy(minimum_score=Decimal("0.5")).evaluate(
        missing_shift, fundamental(), quote()
    )
    ...
    stale_quote = Quote(
        ...
        Decimal("1.09995"),
        Decimal("1.10005"),
        setup.signal_time + timedelta(hours=1),
    )
    result = SignalFusionPolicy(minimum_score=Decimal("0.5")).evaluate(
        setup, fundamental(), stale_quote
    )
    assert result.rejection_code == "QUOTE_STALE"


def test_missing_geometry_does_not_mask_earlier_setup_rejections() -> None:
    setup = technical()
    missing_geometry_and_shift = replace(
        setup,
        stop_reference=None,
    ...
    result = SignalFusionPolicy(minimum_score=Decimal("0.5")).evaluate(
        missing_geometry_and_shift,
        fundamental(),
        quote(),
    )
    assert result.rejection_code == "NO_STRUCTURE_SHIFT"

    low_score_missing_geometry = replace(
        setup,
        score=Decimal("0.40"),
        stop_reference=None,
        take_profit_reference=None,
    ...
    result = SignalFusionPolicy(minimum_score=Decimal("0.5")).evaluate(
        low_score_missing_geometry,
        fundamental(),
        quote(),
    )
    assert result.rejection_code == "SCORE_BELOW_POLICY"

    otherwise_qualified_missing_geometry = replace(
        setup,
        stop_reference=None,
        take_profit_reference=None,
    )
    result = SignalFusionPolicy(minimum_score=Decimal("0.5")).evaluate(
        otherwise_qualified_missing_geometry,
        fundamental(),
        quote(),
    )
    assert result.rejection_code == "NO_STRUCTURAL_TARGET"
...
def test_technical_only_fusion_does_not_add_neutral_fundamental_constant() -> None:
    ...
    spread_penalty = min(
        Decimal("0.08"),
        (Decimal("1") / Decimal("2")) * Decimal("0.04"),
    )
