...
from forex_trader.application.trading_economics_sync import (
    _date_windows,
    sync_trading_economics_fundamentals,
)
...
def _payload(at: datetime = NOW, body: bytes = b"[]") -> RawSourcePayload:
    return RawSourcePayload.create(
        ...
        url="https://api.tradingeconomics.com/calendar/country/All/2026-08-10/2026-08-10",
        body=body,
...
def test_settings_validate_security_and_bounds(monkeypatch: pytest.MonkeyPatch) -> None:
    settings = TradingEconomicsSettings(api_key="client:secret")
    settings.validate()
    with pytest.raises(ValueError, match="API_KEY"):
        TradingEconomicsSettings().validate()
    with pytest.raises(ValueError, match="locked"):
        TradingEconomicsSettings(api_key="x", base_url="https://evil.example").validate()
    with pytest.raises(ValueError, match="TIMEOUT"):
        TradingEconomicsSettings(api_key="x", timeout_seconds=0).validate()
    with pytest.raises(ValueError, match="HISTORY"):
        TradingEconomicsSettings(api_key="x", history_days=91).validate()
    with pytest.raises(ValueError, match="WINDOW"):
        TradingEconomicsSettings(api_key="x", window_days=0).validate()
    with pytest.raises(ValueError, match="MIN_IMPORTANCE"):
        TradingEconomicsSettings(api_key="x", minimum_importance=4).validate()
    with pytest.raises(ValueError, match="AUTH_MODE"):
        TradingEconomicsSettings(api_key="x", auth_mode="basic").validate()
    with pytest.raises(ValueError, match="MAX_PAYLOAD"):
        TradingEconomicsSettings(api_key="x", maximum_payload_bytes=0).validate()
...
def test_parse_calendar_event_rejects_ineligible_or_incomplete_rows() -> None:
    cases = [
        (_row(Country="Atlantis"), "unsupported_currency"),
...
def test_calendar_client_uses_header_auth_and_persists_sanitized_licensed_payload() -> None:
    seen: dict[str, str] = {}
    ...
    settings = TradingEconomicsSettings(api_key="client:secret", history_days=1)
    client = TradingEconomicsCalendarClient(settings, client=http)
    snapshot = client.fetch_calendar(date(2026, 8, 9), date(2026, 8, 10), retrieved_at=NOW)
    assert seen["authorization"] == "Client client:secret"
...
def test_calendar_client_query_auth_sanitizes_persisted_url_and_tracks_skips() -> None:
    ...
    settings = TradingEconomicsSettings(api_key="query-secret", auth_mode="query", history_days=1)
...
def test_calendar_client_fails_closed_for_transport_http_rate_limit_payload_and_shape() -> None:
    settings = TradingEconomicsSettings(api_key="top-secret", history_days=1, maximum_payload_bytes=10)
    ...
    with pytest.raises(TradingEconomicsApiError, match="ReadTimeout"):
        TradingEconomicsCalendarClient(settings, client=http).fetch_calendar(date(2026, 8, 10), date(2026, 8, 10))
    http.close()

    responses = [
        httpx.Response(429, text="limited"),
        httpx.Response(401, text="bad top-secret credential"),
        httpx.Response(200, content=b"not-json", headers={"content-type": "application/json"}),
        httpx.Response(200, json={"wrong": []}),
        httpx.Response(200, content=b"[123456789]"),
    ]
    expected = [
        (TradingEconomicsRateLimitedError, "rate limited"),
        (TradingEconomicsApiError, "\[REDACTED\]"),
        (TradingEconomicsApiError, "valid JSON"),
        (TradingEconomicsApiError, "JSON list"),
        (TradingEconomicsApiError, "maximum size"),
    ]
    for response, (error_type, match) in zip(responses, expected, strict=True):
        http = httpx.Client(transport=httpx.MockTransport(lambda request, response=response: response))
        client = TradingEconomicsCalendarClient(settings, client=http)
        with pytest.raises(error_type, match=match):
            client.fetch_calendar(date(2026, 8, 10), date(2026, 8, 10), retrieved_at=NOW)
        http.close()

    with pytest.raises(ValueError, match="precede"):
        http = httpx.Client(transport=httpx.MockTransport(lambda request: httpx.Response(200, json=[])))
        try:
            TradingEconomicsCalendarClient(settings, client=http).fetch_calendar(date(2026, 8, 11), date(2026, 8, 10))
        finally:
            http.close()
...
def test_calendar_client_rejects_response_host_escape() -> None:
    ...
    with pytest.raises(TradingEconomicsApiError, match="escaped"):
        TradingEconomicsCalendarClient(TradingEconomicsSettings(api_key="x"), client=http).fetch_calendar(
            date(2026, 8, 10), date(2026, 8, 10), retrieved_at=NOW
        )
    http.close()


def test_sync_is_prospective_idempotent_and_records_source_health(tmp_path) -> None:
    ...
    class FakeCalendar:
        ...
        def fetch_calendar(self, start, end, *, retrieved_at=None):  # type: ignore[no-untyped-def]
            assert start == end == NOW.date()
            assert retrieved_at is not None
            return TradingEconomicsCalendarSnapshot(_payload(retrieved_at), (event,), 3, {"low_importance": 2})

    settings = TradingEconomicsSettings(api_key="x", history_days=1, window_days=1)
    first = sync_trading_economics_fundamentals(
        database,
        settings,
        as_of=NOW,
    ...
    assert "prospective-first-seen" in str(first.to_jsonable()["point_in_time_policy"])
...
def test_sync_records_rate_limit_and_unavailable_health(tmp_path) -> None:
    ...
    class LimitedCalendar:
        ...
        def fetch_calendar(self, start, end, *, retrieved_at=None):  # type: ignore[no-untyped-def]
            raise TradingEconomicsRateLimitedError("429", status_code=429)
