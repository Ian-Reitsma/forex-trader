...
1. Request recent economic-calendar windows from `https://api.tradingeconomics.com`.
2. Persist each raw response in `macro_source_payloads` as `SourceAuthority.LICENSED`.
3. Parse supported released macro events for mapped currencies.
4. Require actual, forecast, previous, recognized event semantics, and configured minimum importance.
5. Append immutable `MacroObservation.release` rows to the same SQLite database used by the trading engine.
6. Rebuild the engine from the now-populated point-in-time observation ledger.
7. Run the unchanged fundamental-confidence preflight.
8. Only pairs that pass fundamentals continue to technical, risk, reconciliation, and broker execution gates.
...
The default authentication mode is `header`, which sends:

```text
Authorization: Client <credential>
```

`TRADING_ECONOMICS_AUTH_MODE=query` exists for vendor-plan compatibility. When query authentication is used, persisted provenance URLs strip the `c` credential parameter and raised API errors redact the configured credential.

The base URL is fail-closed to `https://api.tradingeconomics.com`. Redirects are not followed. A response outside the approved host, an oversized payload, malformed JSON, HTTP failure, or rate limit aborts a Practice execution refresh before any strategy order can be submitted.

## Point-in-time policy

This runtime integration is **prospective-first-seen**.

A current Trading Economics response can contain an event that happened days earlier and may already contain revised values. The importer therefore does not backdate that row to the historical release timestamp. The resulting macro observation becomes available at the timestamp the local process first retrieved it. The vendor event ID is used as an immutable first-seen identity, so later refreshes of the same event do not duplicate or rewrite the observation.

This is deliberately conservative. It prevents current/revised vendor data from leaking into an earlier historical decision timestamp. Historical research requiring true as-of consensus/actual evidence must continue to use the repository's dedicated point-in-time historical source paths rather than this prospective runtime feed.
...
Country/currency mapping includes the major OANDA FX currencies and several additional currencies. The downstream strategy still owns eligibility. Missing fundamental confidence, missing macro-factor classification, correlation constraints, spread, technical structure, risk, or execution readiness can all independently veto a trade.
