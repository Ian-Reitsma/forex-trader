    ...
    *,
    ...
    while start + train_size + validation_size <= len(development):
        ...
        validation = development[
            start + train_size : start + train_size + validation_size
        ]
        ...
        validation_report = summarize_trades(
            validation,
    ...
    for threshold in candidate_thresholds:
        ...
        if total_trades == 0:
            continue
        expectancy = sum(
            (report.expectancy_r * Decimal(report.trades) for report in reports),
            Decimal("0"),
...
def validate_multiple_instruments(
    ...
    **kwargs: object,
    ...
    reports = {
        instrument: rolling_threshold_validation(trades, **kwargs)
    ...
    return MultiInstrumentValidation(
        ...
        holdout_win_rate=(
            Decimal("0") if total_trades == 0 else Decimal(total_wins) / Decimal(total_trades)
        ),
        holdout_expectancy_r=(
            Decimal("0") if total_trades == 0 else total_r / Decimal(total_trades)
        ),
        profitable_instruments=sum(
            report.holdout.expectancy_r > 0 for report in reports.values()
        ),
    )
