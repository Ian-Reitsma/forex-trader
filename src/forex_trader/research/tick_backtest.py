...
class TickBacktestOpportunity:
    ...
    def __post_init__(self) -> None:
        if self.decision_time.tzinfo is None or self.entry_time.tzinfo is None or self.exit_time.tzinfo is None:
...
def _last_tick_at_or_before(ticks: Sequence[HistoricalTick], times: Sequence[datetime], instant: datetime) -> HistoricalTick | None:
    index = bisect.bisect_right(times, instant) - 1
    return None if index < 0 else ticks[index]


def _first_tick_at_or_after(ticks: Sequence[HistoricalTick], times: Sequence[datetime], instant: datetime) -> tuple[int, HistoricalTick] | None:
...
def evaluate_candidate_on_ticks(
    ...
    *,
    ...
    if entry_index < 0 or entry_index >= len(ticks):
    ...
    estimated_entry_cost_r = slippage / risk
    ...
    for tick in ticks[entry_index:]:
        ...
        if candidate.direction is Direction.LONG:
            exit_side = tick.bid
            max_favorable = max(max_favorable, max(Decimal("0"), (exit_side - entry_fill) / risk))
            max_adverse = max(max_adverse, max(Decimal("0"), (entry_fill - exit_side) / risk))
            if exit_side <= candidate.stop_loss:
                ...
                trade = BacktestTrade(
                    ...
                    exit_reason="tick_stop",
                    entry_fill=entry_fill,
                    exit_fill=fill,
                    maximum_favorable_r=max_favorable,
                    maximum_adverse_r=max_adverse,
                    estimated_cost_r=estimated_entry_cost_r + slippage / risk,
                )
                return ExactTickOutcome(trade, entry_tick.time, tick.time)
            if exit_side >= candidate.take_profit:
                fill = candidate.take_profit - slippage
                realized = (fill - entry_fill) / risk
                trade = BacktestTrade(
                    candidate.instrument,
                    candidate.direction,
            ...
            exit_side = tick.ask
            max_favorable = max(max_favorable, max(Decimal("0"), (entry_fill - exit_side) / risk))
            max_adverse = max(max_adverse, max(Decimal("0"), (exit_side - entry_fill) / risk))
            if exit_side >= candidate.stop_loss:
    ...
    *,
    ...
    for observation in observations:
        if observation.available_at > as_of:
            break
        if observation.currency in currency_set:
            latest = observation.available_at
    if latest is None:
        return None
    return Decimal(str((as_of - latest).total_seconds() / 60.0))
...
def generate_tick_opportunities(
    *,
    ...
    permissive_policy = SignalFusionPolicy(
        minimum_score=Decimal("0"),
        minimum_fundamental_confidence=Decimal("0"),
        maximum_spread_pips=Decimal("5.0"),
    ...
    for index in range(79, len(lower) - 1):
        ...
        if len(higher_available) < 60:
            continue
        known_tick = _last_tick_at_or_before(ordered_ticks, times, decision_time)
        if known_tick is None or decision_time - known_tick.time > maximum_decision_quote_age:
            continue
        ...
        assessment = fundamentals.assess_pair(normalized, as_of=decision_time)
        known_quote = Quote(normalized, known_tick.bid, known_tick.ask, known_tick.time)
        candidate = permissive_policy.evaluate(technical, assessment, known_quote)
        if candidate.disposition is not DecisionDisposition.TRADE:
            continue

        send = _first_tick_at_or_after(ordered_ticks, times, decision_time + entry_latency)
        if send is None:
            continue
        entry_index, send_tick = send
        ...
        candidate = permissive_policy.revalidate_execution(candidate, send_quote, maximum_spread_pips=Decimal("5.0"))
        if candidate.disposition is not DecisionDisposition.TRADE:
            continue
        assert candidate.entry_price is not None and candidate.stop_loss is not None and candidate.take_profit is not None
        executable_rr = abs(candidate.take_profit - candidate.entry_price) / abs(candidate.entry_price - candidate.stop_loss)
        exact = evaluate_candidate_on_ticks(
            candidate,
            ordered_ticks,
        ...
        directional_news = assessment.differential if candidate.direction is Direction.LONG else -assessment.differential
        ...
        opportunities.append(
            TickBacktestOpportunity(
                ...
                trade=replace(exact.trade, score=technical.score),
                ...
                news_confidence=assessment.confidence,
...
def filter_opportunities(
    ...
    *,
    ...
    next_available_by_instrument: dict[str, datetime] = {}
    for opportunity in sorted(opportunities, key=lambda item: (item.decision_time, item.instrument)):
        ...
        if not _news_allowed(opportunity, strategy_filter):
            continue
        next_available = next_available_by_instrument.get(opportunity.instrument)
        if next_available is not None and opportunity.entry_time < next_available:
            continue
        selected.append(opportunity)
        next_available_by_instrument[opportunity.instrument] = opportunity.exit_time
    return tuple(selected)


def _selection_score(opportunities: Sequence[TickBacktestOpportunity], strategy_filter: StrategyFilter) -> SelectionScore:
    trades = [item.trade for item in opportunities]
    report = summarize_trades(trades, minimum_score=strategy_filter.minimum_score)
    if len(trades) < 2:
        return SelectionScore(strategy_filter, report, Decimal("0"), Decimal("999"))
    mean = report.expectancy_r
    variance = sum(((trade.r_multiple - mean) ** 2 for trade in trades), Decimal("0")) / Decimal(len(trades) - 1)
    standard_error = Decimal(str(math.sqrt(max(0.0, float(variance) / len(trades)))))
...
def default_filter_grid() -> tuple[StrategyFilter, ...]:
    values: list[StrategyFilter] = []
    for score, reward_risk, spread, displacement, session_filter, news_filter in itertools.product(
        ...
        (False, True),
        (SessionFilter.ALL, SessionFilter.LIQUID, SessionFilter.OPEN_ONLY),
        (NewsFilter.NONE, NewsFilter.CONFLICT_VETO, NewsFilter.CONFLICT_VETO_COOLDOWN),
    ):
        values.append(
            StrategyFilter(
                minimum_score=score,
...
def select_filters_on_calibration(
    ...
    *,
    ...
    # This second configuration explicitly targets the user's desired high hit rate but
    # still refuses a negative expectancy or tiny-sample configuration. It is selected
    # on calibration data only and must survive the untouched holdout independently.
...
def simulate_daily_returns(
    ...
    *,
    ...
    daily_start: dict[datetime.date, Decimal] = {}
    daily_end: dict[datetime.date, Decimal] = {}
    ...
    returns = sorted(
        (daily_end[day] / daily_start[day] - Decimal("1") for day in daily_end),
    )
    middle = len(returns) // 2
    median = returns[middle] if len(returns) % 2 else (returns[middle - 1] + returns[middle]) / Decimal("2")
    ...
    return DailyReturnReport(
        ...
        profitable_day_fraction=Decimal(profitable) / Decimal(len(returns)),
...
def evaluate_frozen_filter(
    ...
    *,
    ...
    calibration = filter_opportunities(
        opportunities,
        selection.strategy_filter,
        start=calibration_start,
        end=calibration_end,
    )
    holdout = filter_opportunities(
        opportunities,
        selection.strategy_filter,
    ...
    return WalkForwardSelection(
        strategy_filter=selection.strategy_filter,
        calibration_report=summarize_trades([item.trade for item in calibration]),
        holdout_report=summarize_trades([item.trade for item in holdout]),
        calibration_lower_confidence_expectancy_r=selection.lower_confidence_expectancy_r,
        holdout_daily_returns=simulate_daily_returns(holdout, risk_fraction_per_trade=risk_fraction_per_trade),
        calibration_start=calibration_start,
        calibration_end=calibration_end,
        holdout_start=calibration_end,
