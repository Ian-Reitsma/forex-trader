...
class SignalFusionPolicy:
    ...
    def evaluate(
        ...
        *,
        ...
        # Reject in causal/actionable order. Geometry is checked only after the setup has
        # survived the market-state, confirmation, macro and quality gates. This prevents
        # missing targets from masking the true reason that an immature setup cannot trade.
        if technical.direction is Direction.FLAT:
            return self._abstain(
                technical,
                fundamental,
                "TECHNICAL_FLAT",
                "technical direction is flat",
                reasons,
            )
        ...
        if self.require_liquidity_sweep and not technical.liquidity_sweep:
            return self._abstain(
                ...
                "declared liquidity has not been swept/reclaimed",
                reasons,
            )
        if self.require_structure_shift and not technical.structure_shift:
            return self._abstain(
                ...
                fundamental,
                "NO_STRUCTURE_SHIFT",
                "post-sweep market-structure confirmation is missing",
                reasons,
            )
        if self.require_entry_confirmed and technical.setup_state != SetupState.ENTRY_CONFIRMED.value:
            return self._abstain(
                ...
                f"setup state is {technical.setup_state}, not entry_confirmed",
                reasons,
            )
        if technical.location_score < self.minimum_location_score:
            return self._abstain(
                ...
                f"location score {technical.location_score:.2f} is below {self.minimum_location_score}",
                reasons,
            )
        if self.require_displacement and not technical.displacement:
            return self._abstain(
                ...
                fundamental,
                "NO_DISPLACEMENT",
                "directional displacement confirmation is missing",
                reasons,
            )
        ...
        if self.require_fundamentals and directional_fundamental < -self.maximum_fundamental_conflict:
            return self._abstain(
                ...
                "fundamental context conflicts with direction",
                reasons,
            )

        effective_fundamental = (
            _effective_fundamental(directional_fundamental, fundamental.confidence)
        ...
        if technical.stop_reference is None or technical.take_profit_reference is None:
            return self._abstain(
                ...
                "otherwise-qualified setup lacks executable structural protection/target geometry",
                reasons,
                score=score,
            )
        ...
        if reward_risk < self.minimum_reward_risk:
            return self._abstain(
                ...
                f"executable structural reward/risk {reward_risk:.2f} is below {self.minimum_reward_risk}",
                reasons,
                score=score,
            )
        ...
        *,
        ...
        if candidate.expires_at is not None and quote.time > candidate.expires_at:
            return replace(
                ...
                rejection_code="CANDIDATE_EXPIRED",
                reasons=(*candidate.reasons, "candidate expired before execution"),
                entry_price=None,
                stop_loss=None,
                take_profit=None,
            )
        ...
        if spread_pips > spread_limit:
            return replace(
                candidate,
                disposition=DecisionDisposition.ABSTAIN,
                rejection_code="SPREAD_TOO_WIDE",
                reasons=(*candidate.reasons, f"send-time spread {spread_pips:.2f} pips exceeds {spread_limit}"),
                entry_price=None,
                stop_loss=None,
                take_profit=None,
            )
        ...
        if candidate.direction is Direction.LONG and not candidate.stop_loss < entry < candidate.take_profit:
            return replace(
                candidate,
                disposition=DecisionDisposition.ABSTAIN,
                rejection_code="LATE_ENTRY",
                reasons=(*candidate.reasons, "send-time price consumed the structural trade geometry"),
                entry_price=None,
                stop_loss=None,
                take_profit=None,
            )
        ...
        if rr < self.minimum_reward_risk:
            return replace(
                candidate,
                disposition=DecisionDisposition.ABSTAIN,
                rejection_code="INSUFFICIENT_NET_REWARD",
                reasons=(*candidate.reasons, f"send-time reward/risk fell to {rr:.2f}"),
                entry_price=None,
                stop_loss=None,
                take_profit=None,
            )
        return replace(
            candidate,
            entry_price=entry,
            reasons=(*candidate.reasons, f"send-time execution revalidated at {entry}"),
        )
...
def _execution_key(
    ...
    digest = hashlib.sha256(
        f"{instrument}|{direction.value}|{signal_time}|{setup_family}|{location_id}".encode()
    ).hexdigest()[:24]
